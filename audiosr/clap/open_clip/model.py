...
class CLAP(nn.Module):
    ...
    def audio_infer(self, audio, hopsize=None, device=None):
        """Encode one waveform, optionally using overlapping HTSAT windows."""

        assert not self.training, "the inference mode must be run at eval stage"

        def encode(audio_input, squeeze_batch):
            encoded = self.encode_audio(audio_input, device=device)
            if not squeeze_batch:
                return encoded
            return {
                name: value.squeeze(dim=0) if torch.is_tensor(value) else value
                for name, value in encoded.items()
            }

        if self.audio_cfg.model_type == "PANN":
            return encode(audio.unsqueeze(dim=0), squeeze_batch=True)

        if self.audio_cfg.model_type != "HTSAT":
            raise ValueError(
                f"Unsupported audio model type: {self.audio_cfg.model_type}"
            )
        ...
        repeat_count = self.audio_cfg.clip_samples // audio_len
        if repeat_count > 1:
            audio = audio.repeat(repeat_count)
            audio_len = len(audio)

        if hopsize is None:
            hopsize = min(self.audio_cfg.clip_samples, audio_len)
        elif hopsize <= 0:
            raise ValueError("hopsize must be greater than zero")
        ...
        if audio_len > self.audio_cfg.clip_samples:
            ...
            return encode(torch.stack(audio_input), squeeze_batch=False)
